...
const ModalDeleteTecnico = ({ typeResource,resource, onClose, onDeleteSuccess }) => {
  const handleDelete = async () => {
    try {
      // Realizar la llamada a la API para eliminar el cliente
      ...
      <div className="modal-dialog">
        <div className="modal-content">
          <div className="modal-header">
            <h5 className="modal-title">Eliminar Tecnico</h5>
            <button type="button" className="close" onClick={onClose}>
              <span>&times;</span>
            </button>
          </div>
          <div className="modal-body">
            <p>¿Estás seguro de que deseas eliminar este tecnico?</p>
          </div>
